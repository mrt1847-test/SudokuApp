...
class GameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun newGame(level: String) {
        val generatedSolution = generateSolvedBoard()
        ...
        val puzzleBoard = generatePuzzleBoard(generatedSolution, targetClues)
        applyNewGameState(generatedSolution, puzzleBoard)
    }

    suspend fun newGameAsync(level: String): Boolean {
        isGenerating = true
        ...
        return try {
            ...
            val (generatedSolution, puzzleBoard) = withContext(Dispatchers.Default) {
                val solution = generateSolvedBoard()
                val puzzle = generatePuzzleBoard(solution, targetClues)
                Pair(solution, puzzle)
            }
            applyNewGameState(generatedSolution, puzzleBoard)
            true
        } catch (exception: Exception) {
            loadErrorMessage = "퍼즐 생성에 실패했습니다. 다시 시도해주세요."
            false
        } finally {
            isGenerating = false
        }
    ...
    fun loadGame(): Boolean {
        ...
        return try {
            ...
            loadErrorMessage = "저장된 게임을 불러오지 못했습니다. 초기화가 필요합니다."
            false
        }
    }
    ...
    private fun deserializeBoard(data: String): List<MutableList<Int>> {
        ...
        require(values.size == 81) { "Invalid board data" }
        return List(9) { row ->
    ...
    private fun deserializeBoolBoard(data: String): List<MutableList<Boolean>> {
        ...
        require(values.size == 81) { "Invalid bool board data" }
        return List(9) { row ->
            MutableList(9) { col -> values[row * 9 + col] }
        }
    }
    ...
    private fun deserializeNotes(data: String): List<List<Set<Int>>> {
        ...
        require(parts.size == 81) { "Invalid notes data" }
        return List(9) { row ->
            List(9) { col ->
    ...
    private fun applyNewGameState(
        generatedSolution: List<MutableList<Int>>,
        puzzleBoard: List<MutableList<Int>>
    ) {
        solutionBoard = generatedSolution
        selectedRow = -1
        selectedCol = -1
        selectedNumber = 0
